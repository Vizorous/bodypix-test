...
var blur_1 = require("@tensorflow-models/body-pix/dist/blur");
var util_1 = require("@tensorflow-models/body-pix/dist/util");
...
function toMask(
  ...
  function isSegmentationBoundary(
    ...
    console.log(numberBackgroundPixels);
...
function drawMask(
  ...
  var _a = util_1.getInputSize(image.current),
    height = _a[0],
    width = _a[1];
  canvas.width = width;
  canvas.height = height;
  ...
  if (maskImage) {
    ...
    var blurredMask = drawAndBlurImageOnOffScreenCanvas(
      mask,
      maskBlurAmount,
      CANVAS_NAMES.blurredMask
    );
    // drawOutline(blurredMask, canvas2);
    ctx.drawImage(
      image.current,
      0,
      0,
  ...
  function drawOutline(blurredMask, canvas) {
    ...
    for (; i < dArr.length; i += 2)
      ctx.drawImage(blurredMask, x + dArr[i] * s, y + dArr[i + 1] * s);
...
function createMask(maskImage, maskOpacity, maskBlurAmount) {
  //   console.log(image.current);
...
function drawPixelatedMask(
  ...
  var _a = util_1.getInputSize(image),
    height = _a[0],
    width = _a[1];
  assertSameDimensions(
    { width: width, height: height },
  ...
  var offscreenCanvas = ensureOffscreenCanvasCreated(
...
function createPersonMask(multiPersonSegmentation, edgeBlurAmount) {
  var backgroundMaskImage = toMask(
    multiPersonSegmentation,
    { r: 0, g: 0, b: 0, a: 255 },
    { r: 0, g: 0, b: 0, a: 0 }
  );
  var backgroundMask = renderImageDataToOffScreenCanvas(
    backgroundMaskImage,
    CANVAS_NAMES.mask
  );
  if (edgeBlurAmount === 0) {
...
function drawBokehEffect(
  ...
  var blurredImage = drawAndBlurImageOnOffScreenCanvas(
    image,
    backgroundBlurAmount,
...
function blurBodyPart(
  ...
  var bodyPartMask = createBodyPartMask(
    partSegmentation,
    bodyPartIdsToBlur,
    edgeBlurAmount
  );
